...
// Objects that occupy real city FOOTPRINT: everything except terrain paint and sticky notes.
// (Custom MOC blocks DO count — they stand in for a real build.) The 'Total footprint' in the
// summary, the topbar city-grid readout and the export title card all measure this same set, so
// they can never disagree. Distinct from isCitySet, which also excludes custom blocks (they carry
// no price/piece count) — footprint and purchasing ask different questions.
export function isPhysical(t) { return !!t && t.kind !== 'terrain' && t.kind !== 'note'; }
